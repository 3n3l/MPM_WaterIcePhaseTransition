...
class ThermodynamicModel:
    def __init__(
        ...
        self.gui = ti.GUI("Thermodynamic Model", 720)
    ...
    def render(self):
        ...
        self.gui.circles(
            self.particle_position.to_numpy(),
            radius=1,
            color=0xffffff,
            # palette=self.particle_color,
            # palette_indices=self.particle_phase,
        )
    ...
    def run(self):
        self.reset()
        while not self.gui.get_event(ti.GUI.ESCAPE, ti.GUI.EXIT):
            self.substep()
            self.render()
